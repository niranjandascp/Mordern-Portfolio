...
const SOCIAL_LINKS = [
  ...
  {
    label: "Email",
    href: "mailto:niranjandas@example.com",
    icon: (
  ...
  return (
    <a ref={ref} href={href} aria-label={label} target="_blank" rel="noopener noreferrer"
      ...
      style={{
        ...
        border: hovered ? "1px solid rgba(249,115,22,.7)" : "1px solid rgba(255,255,255,.1)",
        background: hovered ? "rgba(249,115,22,.14)" : "rgba(255,255,255,.04)",
        color: hovered ? "#fdba74" : "rgba(255,255,255,.48)", textDecoration: "none", cursor: "pointer", flexShrink: 0,
...
function NavLink({ href, label }: NavLinkProps) {
  ...
  return (
    <a href={href} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ position: "relative", display: "inline-block", fontSize: 12, letterSpacing: "0.05em", fontWeight: 500, textTransform: "uppercase", textDecoration: "none", paddingBottom: 2, color: hovered ? "#fdba74" : "rgba(255,255,255,.38)", transition: "color .2s" }}>
      {label}
...
function TechPill({ label }: TechPillProps) {
  ...
  return (
    <span onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ fontSize: 10.5, fontWeight: 500, padding: "4px 9px", borderRadius: 50, border: hovered ? "1px solid rgba(249,115,22,.6)" : "1px solid rgba(255,255,255,.09)", background: hovered ? "rgba(249,115,22,.14)" : "rgba(255,255,255,.04)", color: hovered ? "#fdba74" : "rgba(255,255,255,.34)", transition: "all .2s", cursor: "default", letterSpacing: "0.02em" }}>
      {label}
    </span>
  );
}
  ...
  return (
    <>
      ...
      <footer ref={footerRef} style={{ position: "relative", overflow: "hidden", background: "transparent", fontFamily: "'DM Sans',sans-serif", color: "var(--color-text-primary)" }}>
        ...
        {/* HERO */}
        <div style={{ position: "relative", zIndex: 2, textAlign: "center", padding: "64px 20px 52px", borderBottom: "1px solid rgba(251,146,60,.08)" }}>
          <p style={{ fontSize: 11, letterSpacing: "0.2em", color: "rgba(251,146,60,.8)", textTransform: "uppercase", marginBottom: 18, fontWeight: 500 }}>Designed &amp; Developed</p>
          <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "clamp(50px,10vw,104px)", fontWeight: 800, lineHeight: 1, margin: "0 0 20px", background: "linear-gradient(135deg,#fff5f0 0%,#fb923c 30%,#ea580c 55%,#fdba74 80%,#fff5f0 100%)", backgroundSize: "200% 200%", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", animation: "float-kf 5s ease-in-out infinite, shimmer-kf 4s linear infinite", cursor: "default", userSelect: "none", display: "inline-block" }}>
            Niranjan Das
          </h2>
        ...
        {/* Links grid */}
        <div style={{ position: "relative", zIndex: 2, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: "40px 24px", maxWidth: 1040, margin: "0 auto", padding: "48px 28px 44px", borderBottom: "1px solid rgba(251,146,60,.07)" }}>
          {/* Brand */}
          <div>
          ...
          {/* Navigate */}
          <div>
            <p style={{ fontFamily: "'Syne',sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.16em", color: "rgba(251,146,60,.5)", textTransform: "uppercase", marginBottom: 18 }}>Navigate</p>
          ...
          {/* Built with */}
          <div>
            <p style={{ fontFamily: "'Syne',sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.16em", color: "rgba(251,146,60,.5)", textTransform: "uppercase", marginBottom: 18 }}>Built with</p>
          ...
          {/* Connect */}
          <div>
            <p style={{ fontFamily: "'Syne',sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.16em", color: "rgba(251,146,60,.5)", textTransform: "uppercase", marginBottom: 18 }}>Connect</p>
        ...
        {/* Bottom bar */}
        <div style={{ position: "relative", zIndex: 2, maxWidth: 1040, margin: "0 auto", padding: "18px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <span style={{ fontSize: 11, color: "rgba(255,255,255,.2)" }}>Made in India 🇮🇳</span>
            <ScrollTopBtn />
          </div>
        </div>
